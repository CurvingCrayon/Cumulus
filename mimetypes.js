...
//Credit to user rsdoiel at https://github.com/rsdoiel/mimetype-js
//for the exports.data array
var mimeTypes = {
	getExtension: function(mime){
		var ext = false;
		for(var mimeNum = 0; mimeNum < this.data.length; mimeNum++){
			var arr = this.data;
			if(arr[mimeNum].mime_type === mime){
				ext = arr[mimeNum].ext;
				break; //Exit the loop
			}
		}
	...
	getMimeType: function(ext){
		var mime = false;
		for(var extNum = 0; extNum < this.data.length; extNum++){
			var arr = this.data;
			if(arr[extNum].ext === ext){
				mime = arr[extNum].mime_type;
				break; //Exit the loop
			}
		}
		return mime;
